...
public:
    ...
    */
    void readSamples (AudioBuffer<Type> & bufferToFill, int readSize)
    {
        ...
        {
            // If we need to loop around the ring
            if (readPosition.get() + readSize > bufferSize - 1)
            {
                int samplesToEdgeOfBuffer = bufferSize - readPosition.get();
                
                bufferToFill.copyFrom (i, 0, *audioBuffer, i, readPosition.get(),
            ...
            {
                bufferToFill.copyFrom (i, 0, *audioBuffer, i, readPosition.get(), readSize);
            }
        }
    ...
    unsigned int availableSamples () {
        ...
        //if we've wrapped (writePosition has wrapped to be under readPosition)
        if (readPosition.get() > writePosition.get())
        {
            return ((bufferSize - readPosition.get()) + writePosition.get());
        }
        // If we haven't wrapped around the ring
        else
        {
